...
def send_email_otp(email_to):
    ...
    content = (
        ...
        + otp
        + " is your OTP for email verfication.\nThank you."
    )
    ...
    msg.add_alternative(
        ...
        + otp
        + """</h2> is your OTP for email verfication.<br>
                Thank you.
            </body>
        </html>
    """,
...
def verify_vote(private_key, public_key, ballot):
    try:
        ...
        return [
            True,
            "Your vote verfied and Ballot is signed successfully.",
...
def vote_count():
    ...
    vote_result = {
        party: votes.filter(vote_party_id=party).count() for party in parties_id
    }

    return vote_result, final(vote_result)


def final(vote_result):
    won = (None, 0)

    for key, val in vote_result.items():
        if won[0] != key and won[1] < int(val):
            won = (key, val)
    party_name = PoliticalParty.objects.filter(party_id=won[0]).first()
    return {"name": party_name.party_name, "votes": won[1]}
